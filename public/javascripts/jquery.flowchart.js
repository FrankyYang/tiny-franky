...
$.widget( "custom.svgroutes", {
    ...
    _drawMatrix: function (size) {
        ...
        var statusColorConfig = {
            ...
            'Killed': 'rgb(202,105,101)',
            'Failed': 'rgb(202,105,101)',
            'Queued': 'rgb(255,190,135)'
        };
        var rectfill;
        var self = this;
            ...
            if (hoverFlag) {
                ...
                $tip.css('left', $(self.element).position().left + parseInt($dom.attr('x')) + 'px')
                    .css('top', $(self.element).position().top + parseInt($dom.attr('y')) + $(self.element).parent().scrollTop()
                         + parseInt($dom.attr('height')) + 10 + 'px')
                ...
                $.ajax({
                    url: '/process/getWorkflowStepTips?t=' + (new Date()).getTime(),
                    dataType: 'json',
                    method: 'post',
                    ...
                    success: function(data) {
                        if (!hoverFlag) return;
                        if (data.code == 'A00000') {
                            self._setHintContent(data.data, item.stepId, item.dynamicInstanceId);
                        }
                    }
                });
            } else {
    ...
    _setHintContent: function (taskInfo, stepId, dynamicInstanceId) {
